...
int divergence_test(complex, complex, int, double);

int limit = 16;
double eps = 100;
double umin = -1, umax = 1, vmin = -1.2, vmax = 1.2;

int ymin = 0, xmin = 0, ymax = height, xmax = width;
complex c = {0.32, 0.043};
...
int divergence_test(complex z0, complex c, int limit, double eps) {
  complex z;
...
void renderScene() {
  ...
  for (int y = ymin; y <= ymax; y++) {
    for (int x = xmin; x <= xmax; x++) {
      ...
      int n = divergence_test(z0, c, limit, eps);
      if (n == -1) {
